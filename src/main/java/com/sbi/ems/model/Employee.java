 ...
 *   - Many employees → One role         (owning side)
 *   - Many employees ↔ Many projects    (via EmployeeProject join entity)
 */
@Entity
@Table(
    name = "employees",
...
public class Employee {
     ...
     * Scale 2 = two decimal places (e.g. 75000.00).
     * PII field — mask in logs via AOP.
     */
    ...
    /**
     * Employment status — stored as a string enum in DB for readability.
     ...
     * Owning side — holds the FK column "department_id".
     * LAZY fetch — don't load department unless explicitly accessed.
     */
     ...
     * Owning side — holds the FK column "role_id".
     */
    ...
    /**
     * One employee → Many EmployeeProject assignments.
     * Cascade PERSIST + MERGE only — do not delete projects when employee is removed.
     */
    @OneToMany(
        mappedBy = "employee",
